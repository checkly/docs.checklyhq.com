...
- `shouldFail`: A boolean indicating if a response status code > 399 should trigger a failed check, or not. 
- `request` :
  - `url`: The URL to target for your API check.
  - `method`: The HTTP request method to use, e.g. `GET`, `POST`, `PUT` etc.
  - `body`: An HTTP request body as a string.
  - `headers`: An array of key/value pair objects e.g. `{ key: 'Content-Type', value: 'application/json' }`.
  - `queryParameters`: An array of key/value pair objects, e.g. `{ key: 'clientId', value: '12345' }`.
  - `basicAuth` : An object with the shape `{ username: 'admin', password: 'admin' }` to pass as basic auth credentials with the HTTP request.
  - `followRedirects`: A boolean value to indicate if any redirects should be followed.
  - `skipSSL`: A boolean value to indicate whether to validate if an SSL certificate is valid. For dev environments self-signed or expired certificates might be OK.
  - `assertions`: An array of [`AssertionBuilder`](#assertionbuilder) declarations describing assertions on status codes, headers, JSON body contents etc.
  - `bodyType`: A string indicating the type of body payload, `'JSON' | 'FORM' | 'RAW' | 'GRAPHQL' | 'NONE'`. This is typically not needed when using this construct.

#### Doing POST / PUT / PATCH requests 

You can add a body to your API Check request by adding any payload as a string. A common scenario is a JSON payload.

```ts
import { ApiCheck } from '@checkly/cli/constructs'

new ApiCheck('httpbin-org-1', {
  name: 'Post httpbin.org',
  activated: true,
  muted: false,
  frequency: 5,
  request: {
...
- `headers(property?, regex?)`: Assert a set of response headers, takes the header name as the `property` argument and a `regex` to tease out a string from the header value.
...
The example below exports a group as `websiteGroup`...

```ts
// website.check-group.ts
import { CheckGroup, ApiCheck } from '@checkly/cli/constructs'

export const websiteGroup = new CheckGroup('website-group-1', {
  name: 'Website Group',
  activated: true,
  locations: ['us-east-1', 'eu-west-1'],
  tags: ['api-group'],
...
...which is then imported in the API Check defined in another file and assigned to the `group` property of `Check`.

```ts
import { websiteGroup } from './website.check-group.ts'
...
> When adding Browser Checks to a group using `testMatch`, the CLI searches for files using the corresponding [check file](/docs/cli/using-check-test-match/#checkscheckmatch) as a base path.
